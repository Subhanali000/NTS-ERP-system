...
useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      // Fetch only active tasks with manager_id parameter for proper filtering
      ...
      const [tasksData, progressData, userData, allUsers] = await Promise.all([
        tasksRes.json(),
        progressRes.json(),
        usersRes.json(),
        allUsersRes.json(),
...
const myTasks = tasks.filter(
  (task) =>
    task.user_id === user.id || // tasks assigned to the logged-in user (employee)
    (task.manager_id === user.id && task.created_by_director !== null) // tasks created by director for a manager
);


// ✅ Team Tasks: tasks I assigned to my team (not to myself)
const teamTasks = canManageTasks
  ? tasks.filter(
      (t) =>
        t.assigned_by === user.id && t.user_id !== user.id
    )
  : [];
  ...
  const handleCreateTask = async (taskData: any) => {
    ...
    const res = await fetch(`http://localhost:8000/api/${getSimpleDesignation(user.role)}/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
...
useEffect(() => {
  const fetchActiveProjects = async () => {
    ...
    const url = `http://localhost:8000/api/${role.toLowerCase()}/active-projects`;
    console.log(`📡 PROJECTS DEBUG - Fetching from: ${url}`);
    console.log(`📡 PROJECTS DEBUG - User role: ${user.role}`);
    console.log(`📡 PROJECTS DEBUG - Converted role: ${role}`);
    console.log(`📡 PROJECTS DEBUG - Token exists: ${!!token}`);
    ...
    try {
      ...
      console.log(`📡 PROJECTS DEBUG - Response status: ${res.status}`);
      console.log(`📡 PROJECTS DEBUG - Response ok: ${res.ok}`);
      if (!res.ok) {
        const errorText = await res.text();
        console.error("❌ PROJECTS DEBUG - Failed to fetch:", errorText);
        return;
      }

      const body = await res.json();
      console.log("📦 PROJECTS DEBUG - Raw API response:", body);
      console.log("📦 PROJECTS DEBUG - Response type:", typeof body);
      console.log("📦 PROJECTS DEBUG - Has data property:", 'data' in body);

      // Handle different response formats
      // Your backend returns { data: [...] }
      const projectsData = body.data || [];

      console.log("📋 PROJECTS DEBUG - Extracted data:", projectsData);
      console.log("📋 PROJECTS DEBUG - Is array:", Array.isArray(projectsData));
      console.log("📋 PROJECTS DEBUG - Length:", projectsData?.length || 0);
      
      if (projectsData && projectsData.length > 0) {
        console.log("📋 PROJECTS DEBUG - First project:", projectsData[0]);
      }
      setProjects(projectsData);
      console.log("✅ PROJECTS DEBUG - Projects set in state, count:", projectsData.length);
      
      setProjectsFetched(true);
    } catch (err) {
      console.error("❌ PROJECTS DEBUG - Catch error:", err);
      setProjectsFetched(true);
    }
  };
...
// ✅ Log when projects actually updates
useEffect(() => {
  console.log("🧩 PROJECTS DEBUG - State updated:", projects);
  console.log("🧩 PROJECTS DEBUG - State length:", projects?.length || 0);
  console.log("🧩 PROJECTS DEBUG - State is array:", Array.isArray(projects));
  if (projects && projects.length > 0) {
    console.log("🧩 PROJECTS DEBUG - First project in state:", projects[0]);
  }
}, [projects]);
...
/>
        ...
        <div className="flex items-center space-x-1">
  {/* Edit Task - only in 'my-tasks' */}
  {activeTab === 'my-tasks' && (user.id === task.user_id || user.id === task.manager_id) && (
    <button
      onClick={() => {
        setSelectedTask(task.id);
  ...
  {/* Delete Task - only in 'team-tasks' */}
