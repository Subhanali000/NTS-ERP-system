...
const Header: React.FC<HeaderProps> = ({ onLogout }) => {
    ...
    const fetchUser = async () => {
      ...
      if (token) {
        try {
          ...
          setUser({ id: 'c88f4e9a-8bd9-4486-a14d-46fcf7a696fe', name: 'Unknown User', role: 'Global HR Director' });
        }
      } else {
        setUser({ id: 'c88f4e9a-8bd9-4486-a14d-46fcf7a696fe', name: 'Unknown User', role: 'Global HR Director' });
      }
  ...
  // Determine if user is director or manager based on role
  const isDirector = user.role.toLowerCase().includes('director');
  const isManager = user.role.toLowerCase().includes('manager') && !isDirector;
    ...
    if (isDirector || isManager) {
      const pendingLeaveRequests = mockLeaveRequests.filter(lr => {
        ...
        if (isDirector) {
          ...
          return requester.managerId === user.id && requester.role.toLowerCase().includes('manager');
        } else if (isManager) {
      ...
      pendingLeaveRequests.forEach(request => {
        ...
        if (requester) {
          baseNotifications.unshift({
            id: `leave-${request.id}`,
            userId: user.id,
            title: `${isDirector ? 'Manager' : 'Employee'} Leave Request`,
            ...
            actionUrl: isDirector ? '/manager-leave-approvals' : '/leave-management'
          });
        }
      });
    }
        ...
        <div className="flex items-center justify-end h-full px-6">
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              ...
              >
                <img
                  src={mockUsers.find(u => u.id === user.id)?.avatar || 'https://via.placeholder.com/32'}
                  alt={user.name}
